...
if(node[:chef_server_populator][:backup][:remote][:connection])
  ...
  if(remote_bucket && gz_file = remote_bucket.files.get(File.join(node[:chef_server_populator][:backup][:remote][:file_prefix], 'latest.gz')))
    dump_file = remote_bucket.get(remote_bucket.files.get(File.join(node[:chef_server_populator][:backup][:remote][:file_prefix], 'latest.dump')))
