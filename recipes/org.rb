...
execute 'add populator org validator key' do
  ...
  not_if "chef-server-ctl list-client-keys #{org[:name]} #{org[:name]}-validator #{org[:validator_pub_key]} | grep '^key_name: populator$'"
end
...
execute 'remove populator org default validator key' do
  ...
  only_if "chef-server-ctl list-client-keys #{org[:name]} #{org[:name]}-validator #{org[:validator_pub_key]} | grep '^key_name: default$'"
end
